...
describe('views/admin/certificados/index.hbs', () => {
  ...
  it('renderiza filtros com valores selecionados', () => {
    ...
    expect(
      $('select[name="evento_id"] option[selected][value="1"]').length,
    ).toBe(1)
    expect(
      $('select[name="status"] option[selected][value="emitido"]').length,
    ).toBe(1)
    expect($('select[name="tipo_id"] option[selected][value="2"]').length).toBe(
      1,
    )
  })
    ...
    const html = template({
      certificados: [
        ...
        },
        {
          ...
          status: 'pendente',
          nome: 'Cert 2',
          Participante: { nomeCompleto: 'Beltrano' },
          Evento: { nome: 'EV2' },
        },
      ],
      eventos: [],
      ...
      certificados: [
        {
          ...
          status: 'cancelado',
          nome: 'Cert 3',
          Participante: { nomeCompleto: 'Ciclano' },
          Evento: { nome: 'EV3' },
        },
      ],
      eventos: [],
